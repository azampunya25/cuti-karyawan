...
function validasi(form){
  if (form.jdl_gallery.value == ""){
    alert("Anda belum mengisikan judul galeri...!!");
    form.jdl_gallery.focus();
    return (false);
  }
    if (form.album.value == "0"){
    alert("Anda belum memilih album...!!");
    form.album.focus();
    return (false);
  }
  return (true);
...
switch($_GET[act]){
  ...
  default:
    ...
    while($r=mysql_fetch_array($tampil)){
      echo "<tr><td class='left' width='25'>$no</td>
				<td class='center' width='120'><img src='../foto/img_galeri/kecil_$r[gbr_gallery]'></td>
  ...
  case "tambahgalerifoto":
    echo "<h2>Tambah Galeri Foto</h2>
          <form method=POST action='$aksi?module=galerifoto&act=input' enctype='multipart/form-data' onSubmit='return validasi(this)'>
  ...
  case "editgalerifoto":
    ...
    echo "<h2>Edit Galeri Foto</h2>
          <form method=POST enctype='multipart/form-data' action=$aksi?module=galerifoto&act=update enctype='multipart/form-data' onSubmit='return validasi(this)'>
