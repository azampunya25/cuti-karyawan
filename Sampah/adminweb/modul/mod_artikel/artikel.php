...
switch($_GET[act]){
  ...
  default:
    echo "<h2>Artikel</h2>";
	echo" <div class='box'><div class='left'><input type=button value='Tambah Artikel' onclick=\"window.location.href='?module=artikel&act=tambahartikel';\"></div>
		  <div class='right'><form method=get action='$_SERVER[PHP_SELF]'>
          <input type=hidden name=module value=artikel>
          Masukkan Judul Artikel : <input type=text name='kata'> <input type=submit value=Cari>
          </form></div></div>";
    if (empty($_GET['kata'])){
    ...
    while($r=mysql_fetch_array($tampil)){
      ...
      echo "<tr><td class='center' width='25'>$no</td>
                ...
                <td class='left'>$tgl_posting</td>
		            <td class='center' width='85'><a href=?module=artikel&act=editartikel&id=$r[id_artikel]><img src='images/edit.png' border='0' title='edit' /></a>
		                <a href=\"$aksi?module=artikel&act=hapus&id=$r[id_artikel]&namafile=$r[gambar]\" onClick=\"return confirm('Apakah Anda benar-benar mau menghapusnya?')\"><img src='images/del.png' border='0' title='hapus' /></a></td>
		        </tr>";
      $no++;
    }
    echo "</table>";
...
echo "<div class=\"pagination\"> $linkHalaman</div>";
    ...
    while($r=mysql_fetch_array($tampil)){
      ...
      echo "<tr><td class='left'>$no</td>
                <td class='left'>$r[judul]</td>
                <td class='left'>$tgl_posting</td>
		            <td class='left'><a href=?module=artikel&act=editartikel&id=$r[id_artikel]><img src='images/edit.png' border='0' title='edit' /></a>
		                <a href='$aksi?module=artikel&act=hapus&id=$r[id_artikel]&namafile=$r[gambar]'><img src='images/del.png' border='0' title='hapus' /></a></td>
		        </tr>";
      $no++;
    }
    echo "</table>";
...
echo "<div class=\"pagination\"> $linkHalaman</div>";
  ...
  case "tambahartikel":
    ...
    echo "</select></td></tr>
          ...
          <tr><td>Isi Berita</td>  <td> <textarea name='isi_artikel' id='loko' style='width: 600px; height: 350px;'></textarea></td></tr>
  ...
  case "editartikel":
   ...
   if ($r[headline]=='Y'){
      ...
      echo "<tr><td>Isi Artikel</td>   <td> <textarea id='loko' name='isi_artikel' style='width: 600px; height: 350px;'>$r[isi_artikel]</textarea></td></tr>
          <tr><td>Gambar</td>       <td> :  ";
          if ($r[gambar]!=''){
              echo "<img src='../foto/foto_artikel/small_$r[gambar]'>";  
          }
    echo "</td></tr>
