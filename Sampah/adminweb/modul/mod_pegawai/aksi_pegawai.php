  ...
  header('location:../../media.php?module='.$module);
    ...
    mysql_query("INSERT INTO pegawai(nip,
                            ...
                            VALUES('$_POST[nip]',
								   ...
								   '$_POST[status_pegawai]',
								   '$_POST[nip_atasan]')");
	mysql_query("insert into user set nip='$_POST[nip]', password='$_POST[pass]', level='PEGAWAI'");
  header('location:../../media.php?module='.$module);
  }
}
...
elseif ($module=='pegawai' AND $act=='register'){
  ...
  header('location:../../sukses.php');
	...
	mysql_query("insert into user set nip='$_POST[nip]', password='$_POST[psl]', level='PEGAWAI'");
  header('location:../../sukses.php');
  }
}
...
elseif ($module=='pegawai' AND $act=='update'){
	...
	$acak           = rand(1,99);
	$nama_file_unik = $acak.$nama_file; 
	//		$x = "SELECT * FROM pegawai WHERE id_jabatan='$_POST[id_jabatan]'";
	//	$x1 = mysql_query($x);
	//	if (mysql_num_rows($x1) > 0)
//		{
		 //header('location:media.php?err&module='.$module);
		// header('location:../../media.php?errk');
	//	echo "<script>alert('maaf, Kepala Dinas tidak bisa lebih dari satu orang!!');window.location.href='../../media.php?module=pegawai';</script>";
   //      exit();
  //      }
