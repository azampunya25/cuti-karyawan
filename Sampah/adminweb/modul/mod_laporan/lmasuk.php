...
class FPDF_AutoWrapTable extends FPDF {
	...
	public function rptDetailData () {
		...
		$this->SetX($left); $this->Cell(0, 10, 'LAPORAN SURAT MASUK SUBBAG UMUM DAN KEPEGAWAIAN', 0, 1,'C');
		$this->Ln(20);
		...
		//$this->Row(array('No','No. Surat','Tanggal','Nama','Jenis Kelamin','Status Kawin',
			//			 'Agama'));
		...
		$this->SetX($left += 40); $this->Cell(120, $h, 'No Agenda', 1, 0, 'C',true);
		$this->SetX($left += 120); $this->Cell(180, $h, 'Asal Surat', 1, 0, 'C',true);
		$this->SetX($left += 180); $this->Cell(120, $h, 'No Surat', 1, 0, 'C',true);
		$this->SetX($left += 120); $this->Cell(80, $h, 'Tgl Terima', 1, 0, 'C',true);
		$this->SetX($left += 80); $this->Cell(80, $h, 'Tgl Surat', 1, 0, 'C',true);
		$this->SetX($left += 80); $this->Cell(150, $h, 'Perihal', 1, 0, 'C',true);
		$this->SetX($left += 150); $this->Cell(100, $h, 'Disposisi', 1, 1, 'C',true);
		
		$this->SetFont('Times','',12);
		$this->SetWidths(array(40,120,180,120,80,80,150,100));
...
$query = "SELECT * FROM suratmasuk WHERE (((tgl_terima) BETWEEN '".$_POST['dari']."' AND '".$_POST['sampai']."'))";  
