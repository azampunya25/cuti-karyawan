...
switch($_GET[act]){
  ...
  default:
    echo "<h2>Persetujuan Cuti Kepala Dinas</h2>";
     echo"<table class='list'><tr>
			<td class='center' width='50'><img src='images/User-Administrator-Red-icon.png' border='0' title='detail'/><br><b>Kepala Dinas :</td><td width='80'><b>NIP : $_SESSION[namauser]</td> <td><b>Nama : $_SESSION[nama]</td></tr><tbody>";
    echo "<table class='list'><thead>
		  ...
		  <td class='center'>STATUS KABID</th>
		  <td class='center'>STATUS KADIS</th>
    ...
    $tampil=mysql_query("SELECT *FROM permohonan_cuti
	 ...
	 where status_pengajuan='setuju' and status_pengajuankadis='belum'
    ...
    echo"    </td>
				...
				<td class='center'>$r[status_pengajuankadis]</td>
				<td><a href='./aksi.php?module=persetujuan_cutikadis&act=setuju&id_pcuti=$r[id_pcuti]&nip=$r[nip]&nama=$r[nama]' \" onClick=\"return confirm('Apakah Anda yakin untuk meyetujuinya?')\"><img src='images/Like-icon.png' width='25px' height='25px' title='Setuju'></a></td>
	                  <td><a href='./aksi.php?module=persetujuan_cutikadis&act=tdksetuju&id_pcuti=$r[id_pcuti]&nip=$r[nip]&nama=$r[nama]' \" onClick=\"return confirm('Apakah Andayakin tidak menyutujuinya?')\"><img src='images/Unlike-icon.png' width='25px' height='25px' title='Tidak Setuju'></a></td>
		        </tr>";
      $no++;
    }
    echo "</tbody></table>";
